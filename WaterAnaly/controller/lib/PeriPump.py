...
class PeriPump:
    def __init__(self, tca9555_instance, pul_chip=PUL_CHIP, pul_line=PUL_LINE, dir_pin=DIR_PIN, ena_pin=ENA_PIN):
        '''初始化蠕动泵控制器
    ...
    def enable(self):
        '''使能电机'''
        self.tca9555.set_low(self.ena_pin)
        print("电机已使能")
    
    def disable(self):
        '''禁用电机（紧急停止）'''
        self.tca9555.set_high(self.ena_pin)
        print("电机已禁用")
    ...
    def direction(self, direction):
        ...
        '''
        if direction == 1:
            self.tca9555.set_high(self.dir_pin)
        else:
            self.tca9555.set_low(self.dir_pin)
